...
void RunCmd(commandT** cmd, int n)
{
  ...
  if(n == 1)
  {
    ...
    {
       if (cmd[i] -> input_fd != 0)
             close(cmd[i] -> input_fd);
          if (cmd[i] -> output_fd != 1)
             close(cmd[i] -> output_fd);
...
{
   int i, fid[2],  cmdCount ;

   fid[0] = 0;
   fid[1] = 1;
   fd_count = 0;
   for (cmdCount = 0; cmdCount < n ; cmdCount++){
        cmd[cmdCount] -> input_fd = fid[0];
        if(cmdCount != n-1)  {
                 pipe(fid);

                fd_table[fd_count++] = fid[0];
                fd_table[fd_count++] = fid[1];

        }
        else {
                fid[1] = 1;
        }
        cmd[cmdCount] -> output_fd = fid[1];
   }
...
void RunCmdRedirOut(commandT* cmd, char* file)
{
   ...
   // close(1);
   // int fid = open(file, O_WRONLY | O_CREAT | O_TRUNC , S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
  ...
  if (pid == 0)
    {
          if (cmd -> input_fd != 0){
                dup2(cmd -> input_fd, 0);
                close(cmd -> input_fd);     // note!
        }
       if (cmd -> output_fd != 1){
                dup2(cmd -> output_fd, 1);
                close(cmd -> output_fd); // note
        }
       if(cmd -> is_redirect_in){
                fid = open(cmd->redirect_in, O_RDONLY);
                dup2(fid, 0);
                close(fid);
        }
        if (cmd -> is_redirect_out){
                fid = open(cmd->redirect_out, O_WRONLY | O_CREAT | O_TRUNC , S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
                dup2(fid,1);
                close(fid);
        }
         for (i = 0; i < fd_count; i++){
        //   if(i%2 == 0 && fd_table[i] != 0)
        //      close(fd_table[i]);
         //  if(i%2 == 1 && fd_table[i] != 1)
                close(fd_table[i]);
        }
...
void isAlias(commandT** cmd, int n)
{
  ...
  for (i = 0; i < n; i++)
  {
	...
	cmd_n->input_fd = cmd[i]->input_fd;
	cmd_n->output_fd = cmd[i]->output_fd;
      cmd_n->bg = cmd[i]->bg;
      cmd_n->argc = count;
